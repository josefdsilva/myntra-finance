...
// Optional server-side pass (drift and the other nudges). The primary trigger is
// on app-open at 8am local (runDailyCoach); this shares the one runner, so there
// is a single implementation of the coach logic. Idempotent emits.
      ...
      POST: async () => {
        ...
        for (const h of list) {
          try {
            ...
            console.error("budget-alerts failed for", h.id, e);
          }
        }
