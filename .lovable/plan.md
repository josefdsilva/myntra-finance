...
- Ranking: overspend risk > missing data that breaks a number > an unallocated surplus > a stalled
  project > a journey stage one step from done > nothing (show a calm "you're on track").
- Never more than one; the tips list stays below as secondary.

## 3. Weekly rhythm: the cycle review

- A short cycle-end review (3 screens max): what you spent vs estimate, what you set aside, one thing
  to change next cycle. Ends with a single confirm that rolls estimates forward.
- The weekly digest becomes the invitation to that review, not a standalone report.
- Each completed review logs a streak; the streak is the one number we celebrate.

## 4. Proof of progress

- A compact "since you started" strip: cycles reviewed, money set aside, debt reduced, journey level.
  Numbers only from data already computed (cycle metrics, allocations, debts, journey).
- Achievements fire on real milestones (first full cycle, first €500 set aside, first loan overpayment,
  3-cycle streak), surfaced in the coach inbox plus a small toast, never a modal.
- The shareable snapshot is offered right after a milestone — that is when someone wants to share it.

## 5. Bringing people back (notifications that earn their place)

- Three push/email moments only: cycle start (plan), mid-cycle (only if actually drifting), cycle end
  (review). Everything else stays in-app.
- Every notification names a number and links to one screen. If we can't name a number, we don't send.
- Existing notification preferences stay authoritative.
...
- Photo and voice capture surfaced on the dashboard, not only inside the expenses page.

## Sequencing

1. Today card + ranking (highest impact, uses existing signals)
2. Quick-add slimming + one-tap settlement
3. Cycle review + streak
4. Progress strip + milestone achievements
5. Notification trimming to the three moments

## Technical notes

- Today card: new small component used by `dashboard.tsx`; ranking as a pure function next to
  `src/lib/coach-signals.ts` so it is unit-testable. No new tables.
...
- Notification trimming happens in `src/lib/coach-runner.server.ts` (which nudges set push/email), not
  in new endpoints.
- All copy goes through `src/lib/i18n-entries.ts` for every locale.

## Out of scope

- Gamification for its own sake (points, badge walls, leaderboards).
