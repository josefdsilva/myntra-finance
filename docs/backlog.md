...
- AI cost and reliability are load-bearing. Statement import sends whole files to the model; the coach and cycle reports also use AI. Cost work is DONE (2026-07-18): a file-hash cache for statement extraction (`ai_extraction_cache` table; a re-upload of the identical file is free, no AI call, no charge), a trimmed coach snapshot (`slimContext` caps the long arrays before serializing), minified single-line JSON output from the extractor and the statement parser, and verified merchant dedup so a big import never re-hits the AI for merchants it just learned. Still open on reliability: structured / JSON-schema output instead of the regex extractor, retry-once-on-parse-failure with a tolerant repair, chunk long statements (they can truncate), backoff on 429/5xx, and per-row validation that skips bad rows instead of failing the whole import. And observability: a monthly spend rollup per household with an alert threshold, plus a failure-rate counter per operation, on top of the existing per-call metering.
